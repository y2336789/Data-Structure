...
//함수 중에서 return 형이 int인 함수들은 원래 int형 변수 = 함수호출 이런식으로 해야하는게 보통이나, 이처럼 만약에 호출해서 값을 반환 받을 때 이 값이 코드 구현에 큰 의미가 없기에 변수 없이 함수호출을 하였음.
...
int main(void)
{
	...
	free_matrix(matrixA,a,b);	//각각 동적 할당한 행렬들을 free 선언을 해줌으로써 메모리를 반환한다.
	free_matrix(matrixB,a,b);
	free_matrix(summatrix,a,b);
	free_matrix(submatrix,a,b);
	free_matrix(T_matrixA,b,a);
	free_matrix(mulmatrix,a,a);	//동적 할당한 모든 행렬들에 대해 free를 해줘야 한다.
...
void free_matrix(int **matrix, int row, int col)	//동적할당된 2차원 배열을 free선언함으로 메모리를 반환한다.
{
	for(int a = 0; a<row; a++)
